...
abstract class AbstractModel extends DataObject
{
    ...
    public static function details(string $column, $value)
    {
        $tableName = static :: getTableName();
        $sql = "SELECT * FROM {$tableName} WHERE {$column} = :value";
        $statement = Database::getInstance()->prepare($sql);
    ...
    public static function up(string $column,$id, $value)
    {
        $tableName = static :: getTableName();
        $sql = "UPDATE {$tableName} SET {$column}={$column}+{$value} WHERE rev_id ={$id} ";
        $statement = Database::getInstance()->prepare($sql);
        $statement->bindValue('value', $value);
        $statement->execute();
    }
    public static function down(string $column,$id, $value)
    {
        $tableName = static :: getTableName();
        $sql = "UPDATE {$tableName} SET {$column}={$column}-{$value} WHERE rev_id ={$id} ";
        $statement = Database::getInstance()->prepare($sql);
